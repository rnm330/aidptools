(function () {
  ...
  function getScheduleStats() {
    ...
    for (const seg of settings.schedule) {
      ...
      const currentEnd = Math.min(nowMinutes, endM);
      workedMinutes += Math.max(0, currentEnd - startM);
    }

    return { workedMinutes, totalMinutes };
  }

  function expected() {
    const { workedMinutes, totalMinutes } = getScheduleStats();
    if (totalMinutes <= 0) return 0;
    const progressRatio = Math.min(1, Math.max(0, workedMinutes / totalMinutes));
    return Math.floor(progressRatio * settings.target);
  }
  ...
  function updateUI(keepEditing = false) {
    ...
    document.getElementById("target").innerText = String(settings.target);
    document.getElementById("expected").innerText = String(p.exp);

    const diffEl = document.getElementById("diff");
    diffEl.innerText = formatDiff(p.diff);

    const doneRatio = settings.target > 0 ? Math.min(1, Math.max(0, p.done / settings.target)) : 0;
    const expectedRatio = settings.target > 0 ? Math.min(1, Math.max(0, p.exp / settings.target)) : 0;
